...
if ( ! $only_membership ) :
	...
	<a href="#" <?php echo wp_kses_post( implode( ' ', apply_filters( 'stm_lms_buy_button_auth', $attributes, $post_id ) ) ); ?>>
		<span class="masterstudy-buy-button__title"><?php esc_html_e( 'One Time Payment', 'masterstudy-lms-learning-management-system' ); ?></span>
	</a>
	<?php
endif;

if ( $subscription_enabled && ! empty( $plans_courses ) ) :
	echo '<span class="masterstudy-buy-button_plans-dropdown__title">' . esc_html__( 'Available in this plans:', 'masterstudy-lms-learning-management-system' ) . '</span>';
	...
	if ( $plans_have_quota ) :
		...
		<button type="button" data-masterstudy-modal="masterstudy-membership-modal">
			<span class="masterstudy-buy-button__title"><?php esc_html_e( 'Enroll with Membership', 'masterstudy-lms-learning-management-system' ); ?></span>
		</button>
		<?php
